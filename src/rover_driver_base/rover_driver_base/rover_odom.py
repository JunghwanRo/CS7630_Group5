...
class RoverDriver(Node):
    ...
    def sync_odo_cb(self, *args):
        ...
        motors = RoverMotors()
        # Debug: get logger info for timestamp of motors
        # self.get_logger().info("Timestamp of motors: " + str(motors.timestamp))
    ...
    def odo_cb(self, timestamp, motors):
        ...
        if self.publish_tf:
            ...
            t.transform.translation.x = -(
                cos(X[2, 0]) * X[0, 0] + sin(X[2, 0]) * X[1, 0]
            )
            t.transform.translation.y = -(
                -sin(X[2, 0]) * X[0, 0] + cos(X[2, 0]) * X[1, 0]
            )
